...
async function onTabsUpdated(tabId, changeInfo, tabInfo) {
  ...
  if (marks.has(tabId) && marks.get(tabId).url !== tabUrl) {
    ...
    const bmark = await browser.bookmarks.search({
...
browser.menus.onShown.addListener(async function (info /*, tab*/) {
  if (info.bookmarkId) {
    ...
    if (!bmn.url) {
      ...
      browser.menus.update(extname, {
        visible: true,
        checked: tmp === info.bookmarkId,
      });
    } else {
...
function traverse_and_upgrade(bookmarkItem) {
  if (bookmarkItem.url) {
    ...
    if (bookmarkItem.url.endsWith(postfix)) {
      // add id/epochtime
      browser.bookmarks.update(bookmarkItem.id, {
        url: bookmarkItem.url + makeUpgradeId(11),
      });
    }
  }
